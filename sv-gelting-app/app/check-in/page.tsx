...
export default function CheckInPage() {
  ...
  const card: React.CSSProperties = { border: "2px solid #111", borderRadius: 18, padding: 18, background: "#fff", maxWidth: 900, marginTop: 16 };
  ...
  return (
    <main style={{ padding: 24 }}>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <Link
          href="/termine"
          style={{ border: "2px solid #111", borderRadius: 999, padding: "10px 14px", fontWeight: 900, textDecoration: "none", color: "#111", background: "#fff" }}
        >
          ← Zu den Terminen
        </Link>
      </div>
      ...
      {!termin ? null : (
        <div style={card}>
          <div style={{ fontWeight: 950, fontSize: 20 }}>
            Anwesenheit: <span style={{ opacity: 0.75 }}>{presentCount} / {teamPlayers.length}</span>
          </div>
              ...
              return (
                <div key={p.id} style={row}>
                  <div style={{ fontWeight: 950 }}>
                    {p.number ? `#${p.number} ` : ""}{p.name} <span style={{ opacity: 0.7 }}>({p.position})</span>
                  </div>

                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
                    <button style={btn} onClick={() => setAtt(p.id, true)}>✅ war da</button>
                    <button style={btn} onClick={() => setAtt(p.id, false)}>❌ nicht da</button>
                    <button style={btn} onClick={() => clearAtt(p.id)}>⟲ offen</button>
